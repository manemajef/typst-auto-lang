...
`auto-bidi` scans the characters in each paragraph, heading, list, or enum and picks the dominant script. A paragraph with more Hebrew characters than Latin ones → Hebrew → RTL. Majority wins.

```typst
// mostly Hebrew → RTL
גם אם $f(x) = x^2$ מופיע באמצע, הפסקה הזאת עברית.

// mostly English → LTR
Even if a little עברית appears here, this paragraph is English.
```

### "first" mode

Prefer the behaviour of Apple Notes and WhatsApp, where the **first character** decides direction:
...
The direction of a list or enum is determined by the list as a whole, not item by item. A Hebrew list stays RTL even if one item contains an English word:

```typst
- פריט ראשון
- second item — still RTL because the list is Hebrew
- פריט שלישי
```
